    ...
    <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-4xl mt-10">
        ...
        <form action="{{ route('periode.update', $periode->id_periode) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_periode" id="nama_periode" class="w-full p-2 border border-gray-300 rounded" value="{{ old('nama_periode', $periode->nama_periode) }}">
            </div>
            <div class="mb-4">
                <label for="tanggal_mulai" class="block text-gray-700">Tanggal Mulai:</label>
                <input type="date" name="tanggal_mulai" id="tanggal_mulai" class="w-full p-2 border border-gray-300 rounded" value="{{ old('tanggal_mulai', $periode->tanggal_mulai) }}">
            </div>
            <div class="mb-4">
                <label for="tanggal_selesai" class="block text-gray-700">Tanggal Selesai:</label>
                <input type="date" name="tanggal_selesai" id="tanggal_selesai" class="w-full p-2 border border-gray-300 rounded" value="{{ old('tanggal_selesai', $periode->tanggal_selesai) }}">
            </div>
            <div>
